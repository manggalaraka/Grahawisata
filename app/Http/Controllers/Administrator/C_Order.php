...
class C_Order extends Controller
{
	...
	protected function step1(Request $request,$id_kelas,$nama,$data_reservasi){
		...
		// return redirect('/home/pesan_kamar/booking');
	...
	protected function add(Request $request){
		if(session('time_book_expired')>=date("d-m-Y H:i:s")){
			...
			$day_start = ($this->validateDate(session('waktu')))->format('Y-m-d')." 00:00:00";
			$day_end =  date('Y-m-d',strtotime(($this->validateDate(session('waktu')))->format('Y-m-d').' + '.session('durasi').' days'))." 00:00:00";
		   ...
		   $request->merge(array('id_pesanan'=>M_order::mgenerate_id(),
			...
			// // echo json_encode($this->generate_promo("Lebaran15",session('id_kelas')))."</br>";
			// echo json_encode($request->all())."</br>"; 
			// echo json_encode($request->session()->all())."</br>";
			// echo json_encode(M_order::mgenerate_id())."</br>";
			// echo json_encode(session('total_harga'))."</br>";
			// exit();
